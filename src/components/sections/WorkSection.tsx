    ...
    >
      <BackgroundBlobs
        blobs={[
          {
            className:
              "absolute top-0 right-0 w-48 h-48 md:w-96 md:h-96 bg-blue-200/25 dark:bg-blue-900/10 rounded-full blur-3xl",
            animate: { x: [0, 30, 0], y: [0, -20, 0] },
            transition: { duration: 14, repeat: Infinity, ease: "easeInOut" },
          },
          {
            className:
              "absolute bottom-0 left-0 w-48 h-48 md:w-96 md:h-96 bg-purple-200/25 dark:bg-purple-900/10 rounded-full blur-3xl",
                  ...
                  >
                    <TiltCard maxTilt={8} scale={1.01} className="h-full">
                      ...
                      >
                        {/* Watermark number */}
                        <span
                          className={`absolute -top-4 -right-2 text-6xl md:text-8xl font-black pointer-events-none select-none leading-none ${accent.num}`}
                        >
                          {num}
                        </span>
                        ...
                        <div
                          className={`absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t ${accent.glow} to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none`}
                        />

                        {/* Header row */}
                        <div className="flex items-start gap-4 relative z-10">
                          <div
                            className={`flex-shrink-0 p-3 rounded-xl bg-gradient-to-br ${accent.gradient} shadow-md`}
                          >
                            <Icon className="w-6 h-6 text-white" />
                          </div>
                          <div className="flex flex-col gap-1 min-w-0">
                            <h3
                              className={`font-bold text-lg leading-tight bg-gradient-to-r ${accent.gradient} bg-clip-text text-transparent`}
                            >
                              {project.name}
                            </h3>
                            {metric && (
                              <span
                                className={`inline-flex w-fit text-[11px] font-bold px-2 py-0.5 rounded-full bg-gradient-to-r ${accent.gradient} text-white`}
                              >
                                ⚡ {metric.trim()}
                              </span>
                            )}
                        ...
                        {/* Divider */}
                        <div
                          className={`h-px bg-gradient-to-r ${accent.gradient} opacity-20`}
                        />
